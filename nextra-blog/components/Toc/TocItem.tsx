...
export function TocItem({ item }: TableOfContentsProps) {
  ...
  return <Box onClick={() => setActive(item.depth)} onMouseEnter={() => setActive(item.depth)} onMouseLeave={() => setActive(0)}>
    <Link
      href={`#${item.id}`}
      key={item.value}
      className={cx(classes.link, { [classes.linkActive]: active === item.depth })}
      style={{
